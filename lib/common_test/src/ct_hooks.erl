...
-spec groups(Mod :: atom(), Groups :: term()) ->
    term().
groups(Mod, Groups) ->
    ...
    call(fun call_generic/3, Info ++ Groups, [post_groups, Mod]).

%% @doc Called for the Mod:all/0 function
-spec all(Mod :: atom(), Tests :: atom() | {'group',atom()}) ->
    atom() | {'group',atom()}.
all(Mod, Tests) ->
    call(fun call_generic_fallback/3, Tests, [post_all, Mod]).
...
%% If CTH is started because of a call from groups/2, CTH:init/1 must not be
%% called (the suite scope should be used).
call_init(Hook, Config, [GroupsOrAll|_Meta]) when GroupsOrAll == post_groups;
                                                  GroupsOrAll == post_all ->
    {Config,Hook};
